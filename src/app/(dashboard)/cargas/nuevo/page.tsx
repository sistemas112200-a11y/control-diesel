...
export default async function NuevaCargaPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <form action={crearCargaAction} className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
      ...
      <form action={crearCargaAction} className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Select
            ...
            options={vehiculos.map((v) => ({ value: v.id, label: v.numero_economico }))}
          />
          <Select label="Operador" name="operador_id" required options={(operadores ?? []).map((o) => ({ value: o.id, label: o.nombre_completo }))} />
        </div>
        <div className="grid grid-cols-2 gap-4">
