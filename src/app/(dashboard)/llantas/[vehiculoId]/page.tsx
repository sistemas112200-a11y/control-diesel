...
import { getLlantas, type PosicionLlanta, type Llanta } from '@/repositories/llanta.repository'

const ETIQUETAS_POSICION: Record<PosicionLlanta, string> = {
...
export default async function LlantasVehiculoPage({
  ...
  const porPosicion = new Map<PosicionLlanta, Llanta>()
  ...
  function Llantita({ posicion }: { posicion: PosicionLlanta }) {
    ...
    return (
      <Link
        ...
        title={ETIQUETAS_POSICION[posicion]}
      >
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-8">
        <div className="max-w-sm mx-auto flex flex-col items-center gap-6">
          <div className="flex items-center gap-16">
            <Llantita posicion="delantera_izquierda" />
            <Llantita posicion="delantera_derecha" />
          </div>

          <div className="w-24 h-28 rounded-2xl bg-slate-100 border border-slate-300 flex items-center justify-center">
            <span className="text-slate-400 text-xs text-center px-2">Tracto</span>
          </div>

          <div className="flex items-center gap-4">
            <Llantita posicion="trasera_izquierda_externa" />
            <Llantita posicion="trasera_izquierda_interna" />
            <div className="w-6" />
            <Llantita posicion="trasera_derecha_interna" />
            <Llantita posicion="trasera_derecha_externa" />
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {extras.map((l) => {
                ...
                return (
                  <tr key={l.id}>
                    <td className="px-4 py-3 text-slate-600">{ETIQUETAS_POSICION[l.posicion as PosicionLlanta]}</td>
